...
<td align="left"><a href="<?php if($status==1) { ?>?action=edit&itemid=<?php echo $v['itemid'];?><?php } else { ?>?action=show&itemid=<?php echo $v['itemid'];?><?php } ?>
"<?php if($status>2 && !$v['isread']) { ?> class="f_b"<?php if($v['feedback']) { ?>onclick="if(confirm('该信件请求发送已读回执，是否发送？')){ Go(this.href+'&feedback=1');return false;}"<?php } ?>
<?php } ?>
<?php if($v['style']) { ?> style="color:#<?php echo $v['style'];?>;"<?php } ?>
 class="t" title="<?php echo $v['title'];?>&#10;编号：<?php echo $v['itemid'];?>"><?php echo $v['dtitle'];?></a></td>
<?php if($status>1) { ?><td align="center"><?php if($v['userurl']) { ?><a href="<?php echo $v['userurl'];?>" target="_blank"><?php echo $v['user'];?></a><?php } else { ?><?php echo $v['user'];?><?php } ?>
</td><?php } ?>
<td><?php echo $v['adddate'];?></td>
<?php if($status==3) { ?>
<td>
